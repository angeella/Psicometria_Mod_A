...
Prima di tutto.... **Aprire Rstudio**! 

Abbiamo 4 finestrelle: 

<center>
![](Images/Rstudio.JPG){ width=80% }
</center> 
...
Tramite questi comandi, installiamo il pacchetto ``xlsx`` e lo richiamiamo nella nostra sessione. Questo pacchetto ci permette di caricare all'interno di R dati da Excel.
...
significa che dovete installare il pacchetto ``dplyr``. Consiglio `r emo::ji("face")`: errori/bug cercate su google o [stackoverflow](https://stackoverflow.com)!
...
Attenzione `r emo::ji("boom")`! ``=`` e ``<-`` si usano per assegnare un valore a una variabile
...
Che differenza c'&egrave tra `data.frame` e `matrix`? `r emo::ji("question")`
...
```{r, echo = T}
barplot(fa_h, col = "lightblue")
```

## Variabili qualitative | Grafici
<br>
Grafico a torta 
<br>
```{r, echo = T}
pie(fa_h, col = "lightblue")
```
...
Istogramma
```{r, echo = T, eval = F}
hist(DB$HappinessScore, main = "Happiness Score", col = "lightblue")
hist(DB$HappinessScore, breaks = 20, col = "lightblue") #con breaks decidi gli intervalli!
```
